...
async def edit_patients(request: Request, id: int ):
    async with async_session() as db:
        result = await db.get(Patient, id)
    return templates.TemplateResponse("edit_patients.html", {
        "request": request,
        "patient": result
    })
...
@app.get("/delete/{id}", clas_response=HTMLResponse)
async def delete_patients(request: Request, id: int):
    async with async_session() as db:
        patient = await db.get(patient, id)
        if patient:
            await db.delete()
    ...
    return templates.TemplateResponse("delete_patients.html", {
        "request": Request
